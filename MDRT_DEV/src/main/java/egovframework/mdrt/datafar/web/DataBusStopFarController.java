...
public class DataBusStopFarController {
	
	
	
	@Resource(name = "DataBusStopFarService")
	protected DataBusStopFarService dataBusStopFarService;
	
		
	@RequestMapping(value = "/dataBusStopFar.json")
	public ModelAndView dataBusStopFarData(HttpServletRequest request,HttpSession session, @RequestBody UserVo userSearch) throws Exception {
		ModelAndView modelAndView = new ModelAndView();
		try {
		
		List<UserVo> resultSummary = dataBusStopFarService.getDataBusStopFar(userSearch);           //사용자 조회
